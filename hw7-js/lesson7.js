...
var block_timer; // Таймер появления блоков
var BLOCK_TIME = 5000;
...
function prepareGameField() {
    ...
    scr.id = 'score';
    scr.innerText = 'Счет: 0';
    document.getElementById('snake-field').appendChild(scr); // Добавление счета
...
function startGame() {
    ...
    block_timer = setInterval(createBlock, BLOCK_TIME);
...
function move() {
    ...
    else if ( direction == 'y+' && new_unit == undefined ) {
        coord_y = 19;
    ...
    else if ( direction == 'x+' && new_unit == undefined ) {
        coord_x = 0;
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x))[0];
        new_unit.setAttribute('class', new_unit.getAttribute('class') + ' snake-unit');
        snake.push(new_unit);
...
function isBroke(unit) {
    var check = false;

    var unit_classes = unit.getAttribute('class').split(' ');

    // Если наехала
    if (unit_classes.includes('block_unit')) {
        check = true;
    }
    return check;
}
...
function createBlock() {
    ...
    var block = document.getElementsByClassName('block_unit');

    for (var c = 0; c < block.length; c++) {
        block[c].classList.remove('block_unit');
    }
        ...
        if (!block_cell_classes.includes('snake-unit')) {
            ...
            block_cell.setAttribute('class', classes + 'block_unit');
            var snake_head_classes = snake[snake.length-1].getAttribute('class').split(' ');
